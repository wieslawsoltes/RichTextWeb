...
export async function runWordAuthoringBrowserChecks(page) {
  ...
  const initial = await page.evaluate(() => {
    ...
    return {
      ...
      arrangement: editor.PageArrangement,
  ...
  try {
    ...
    await page.evaluate(async (initial) => {
      ...
      toolbar?.Dispose();
      toolbar?.remove();
      // Flush responsive callbacks before restoring desktop zoom and docking.
      // Otherwise the next suite can see a stale mobile zoom and race pane hiding.
      ...
      editor.DocumentView = initial.view;
      editor.PageArrangement = initial.arrangement;
      workspace.Docking.LoadLayout(initial.layout);
      workspace.SetZoom(initial.zoom * 100);
      editor.ZoomMode = initial.zoomMode;
      editor.Select(initial.start, initial.end);
